      ...
      <form class="card forms-sample" action="{{isset($item->id) ?  route('dana_penelitian.update', ['id' => Crypt::encryptString($item->id)])  : route('dana_penelitian.store')}}" method="post">
        ...
        <div class="card-body">
          ...
          <div class="form-group row">
            ...
            <div class="col-sm-9">
              <input type="text" name="judul_penelitian" value="{{ isset($item->judul_penelitian) ? $item->judul_penelitian : old('judul_penelitian') }}" class="form-control" placeholder="Ketik disini">
            </div>
          </div>
          <div class="form-group row">
              ...
              <select class="form-control" name="nidn_nidk">
                ...
                @foreach($dosens as $dosen)
                  <option value="{{ $dosen->nidn_nidk }}"
                    @if(old('nidn_nidk', isset($item->nidk_nidk) ? $item->nidk_nidk : '') == $dosen->nidn_nidk) selected @endif>
