...
<div class="content-wrapper pb-0">
  ...
  <div class="row">
    <div class="col grid-margin stretch-card">
      <form class="card forms-sample" action="{{isset($item->id) ?  route('pelibatan_mahasiswa_dalam_penelitian.update', ['id' => Crypt::encryptString($item->id)])  : route('pelibatan_mahasiswa_dalam_penelitian.store')}}" method="post">
        @if(isset($item->id))
        @method('PUT')
        @endif
        @csrf
  ...
  <div class="col-sm-9">
    <input type="number" name="tahun_akademik" value="{{ old('tahun_akademik', isset($item->tahun_akademik) ? $item->tahun_akademik : '') }}" class="form-control @error('tahun_akademik') is-invalid @enderror" placeholder="Ketik disini">
...
<div class="form-group row">
  ...
  <div class="col-sm-9">
    <input type="text" name="kepakaran_ketua" value="{{ old('kepakaran_ketua', isset($item->kepakaran_ketua) ? $item->kepakaran_ketua : '') }}" class="form-control @error('kepakaran_ketua') is-invalid @enderror" placeholder="Ketik disini">
...
<div class="form-group row">
  ...
  <div class="col-sm-9">
    <input type="text" name="mahasiswa" value="{{ old('mahasiswa', isset($item->mahasiswa) ? $item->mahasiswa : '') }}" class="form-control @error('mahasiswa') is-invalid @enderror" placeholder="Ketik disini">
    @error('mahasiswa')
      <div class="invalid-feedback">{{ $message }}</div>
    @enderror
  </div>
